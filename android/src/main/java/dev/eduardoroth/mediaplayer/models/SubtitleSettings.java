...
public class SubtitleSettings implements Serializable {
    ...
    private int getColorFromRGBA(String rgbaColor, int defaultColor) {
        if (rgbaColor.length() > 4 && rgbaColor.startsWith("rgba")) {
            ...
            if (colors.size() == 4) {
                ret = (Math.round(Float.parseFloat(colors.get(3).trim()) * 255) & 0xff) << 24 | (Integer.parseInt(colors.get(0).trim()) & 0xff) << 16 | (Integer.parseInt(colors.get(1).trim()) & 0xff) << 8 | (Integer.parseInt(colors.get(2).trim()) & 0xff);
            }
            return ret;
        }
        return defaultColor;
